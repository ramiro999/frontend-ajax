...
const LoginForm = () => {
  ...
  const login = async (e) => {
    ...
    try {
      const { status } = await userLogin(userData);
      if (status === 200) {
        // Logica de éxito
        ...
        <div class="grid md:grid-cols-2 items-center gap-4 max-w-7xl w-full">
          <div class="border border-gray-300 rounded-md p-6 max-w-md shadow-[0_2px_22px_-4px_rgba(93,96,127,0.2)] max-md:mx-auto">
            <form class="space-y-6" onSubmit={login}>
              <div class="mb-10">
                <h3 class="text-3xl font-extrabold">Inicia sesión</h3>
                <p class="text-sm mt-4">
                  Inicia sesión y disfruta de nuestros servicios. !Eres nuestro
                  mejor aliado!
                </p>
              </div>
              <div>
                <label class="text-sm mb-2 block">Usuario</label>
